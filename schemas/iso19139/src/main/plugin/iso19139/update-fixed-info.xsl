...
<!--
     ...
     ~ Modfied 2019-04-16 Thomas Vandenberghe: 
     disable the keyword improvements, line 28, <xsl:include href="update-fixed-info-keywords.xsl"/>
     remove the whole gco:CharacterString|gmx:Anchor|gmd:PT_FreeText template
     disable the template that deletes translated elements if the translated language si not the PT_Locale list:  <xsl:template match="gmd:textGroup"> !!
-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gml="http://www.opengis.net/gml" xmlns:srv="http://www.isotc211.org/2005/srv" xmlns:gmx="http://www.isotc211.org/2005/gmx" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:gn-fn-iso19139="http://geonetwork-opensource.org/xsl/functions/profiles/iso19139" xmlns:geonet="http://www.fao.org/geonetwork" xmlns:java="java:org.fao.geonet.util.XslUtil" version="2.0" exclude-result-prefixes="#all">
  ...
  <xsl:variable name="mainLanguage" select="/root/*/gmd:language/gco:CharacterString/text()|
                        /root/*/gmd:language/gmd:LanguageCode/@codeListValue" />
  <xsl:variable name="isMultilingual" select="count(/root/*/gmd:locale[*/gmd:languageCode/*/@codeListValue != $mainLanguage]) > 0" />
  <xsl:variable name="mainLanguageId" select="upper-case(java:twoCharLangCode($mainLanguage))" />
  <xsl:variable name="locales" select="/root/*/gmd:locale/gmd:PT_Locale" />
  <xsl:variable name="defaultEncoding" select="'utf8'" />
  <xsl:variable name="editorConfig" select="document('layout/config-editor.xml')" />
  <xsl:variable name="nonMultilingualFields" select="$editorConfig/editor/multilingualFields/exclude" />
      ...
      <xsl:if test="$isMultilingual">
        ...
        <gmd:locale>
          <gmd:PT_Locale id="{$mainLanguageId}">
            <gmd:languageCode>
              <gmd:LanguageCode codeList="http://www.loc.gov/standards/iso639-2/" codeListValue="{$mainLanguage}" />
            </gmd:languageCode>
            <gmd:characterEncoding>
              <gmd:MD_CharacterSetCode codeList="http://standards.iso.org/ittf/PubliclyAvailableStandards/ISO_19139_Schemas/resources/codelist/ML_gmxCodelists.xml#MD_CharacterSetCode" codeListValue="{$defaultEncoding}" />
            </gmd:characterEncoding>
          </gmd:PT_Locale>
        </gmd:locale>
      </xsl:if>
  ...
  <!--     
       ...
       <xsl:variable name="elementLocalId" select="replace(gmd:LocalisedCharacterString/@locale, '^#', '')" />
       <xsl:choose>
       <xsl:when test="count($locales[@id = $elementLocalId]) > 0">
       <gmd:textGroup>
       <gmd:LocalisedCharacterString>
       <xsl:variable name="currentLocale" select="replace(gmd:LocalisedCharacterString/@locale, '^#', '')" />
       <xsl:variable name="ptLocale" select="$locales[@id = string($currentLocale)]" />
       ...
       <xsl:otherwise>-->
  <!--<xsl:message>Removing <xsl:copy-of select="."/>. This element was removed because not declared in record locales.</xsl:message>-->
  <!--
       </xsl:otherwise>
       </xsl:choose>
       </xsl:template>-->
