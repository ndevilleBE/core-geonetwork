...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gmd="http://www.isotc211.org/2005/gmd"
  ...
  <xsl:template mode="schema-hidden-fields"
                match="gmd:MD_Metadata|*[@gco:isoType='gmd:MD_Metadata']" priority="2">
  ...
  <!--
    ...
    -->
  <xsl:template mode="iso19139"
                match="srv:operatesOn|gmd:featureCatalogueCitation|gmd:source[name(parent::node())='gmd:LI_ProcessStep' or name(parent::node())='gmd:LI_Lineage']"
                priority="99">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
    <xsl:variable name="text">
      ...
      <xsl:choose>
        <xsl:when test="$edit=true()">
          ...
          <xsl:variable name="typeOfLink"
                        select="if (local-name(.)='featureCatalogueCitation') then 'iso19110' else 'uuidref'"/>
 ...
 Display a list of related resources to which the current service metadata operatesOn.
      ...
      <xsl:when test="$edit=true()">
        <xsl:variable name="text">
          ...
          <xsl:choose>
            ...
            <xsl:otherwise>
              <select
                onchange="javascript:Ext.getDom('_{$ref}').value=this.options[this.selectedIndex].value;"
                class="md">
                <option></option>
    ...
    <xsl:variable name="text">
      <xsl:choose>
        <xsl:when test="$edit=true()">
          ...
          <input type="text" id="Y{$ref}" class="small"
            ...
            <xsl:variable name="link">
              <xsl:choose>
                <xsl:when test="geonet:is-image(gmx:FileName/@src)">
                  <div class="logo-wrap">
                    <img class="logo" src="{gmx:FileName/@src}"/>
                  </div>
                </xsl:when>
                <xsl:otherwise>
      ...
      <xsl:choose>
        <xsl:when test="geonet:element/@max > 1">
          <xsl:variable name="codelist"
                        select="geonet:getCodelist($schema, name(*[@codeListValue]), /root/gui)"/>
          <xsl:value-of
            select="if ($codelist/codelist/@editorMode='select') then true() else false()"/>
        </xsl:when>
        <xsl:otherwise>
    ...
    <xsl:choose>
      <xsl:when test="$multiple=true()">
        <xsl:variable name="firstElement"
                      select="count(preceding-sibling::node()[name()=$elementName])=0"/>
  ...
  <xsl:template name="iso19139CodelistMultiple">
    ...
    <xsl:variable name="codelist"
                  select="geonet:getCodelist($schema, $codeListElementName, /root/gui)"/>

    <xsl:apply-templates mode="simpleElement" select=".">
      <xsl:with-param name="schema" select="$schema"/>
      <xsl:with-param name="edit" select="$edit"/>
      <xsl:with-param name="refs"
                      select="concat(geonet:element/@ref, ',', string-join(following-sibling::node()[name()=$elementName]/geonet:element/@ref, ','))"/>
      <xsl:with-param name="text">
        ...
        <select id="{geonet:element/@ref}" class="md codelist_multiple" multiple="multiple"
                size="{if($max &lt; 7) then $max else '7'}">
          <xsl:if test="$edit">
          ...
          <xsl:for-each select="$codelist/codelist/entry[not(@hideInEditMode)]">
            ...
            <option>
              <xsl:if
                test="count($element/../node()[name()=$elementName]/child::node()[@codeListValue=$code])!=0">
          ...
          <xsl:for-each
            select="../node()[name()=$elementName]/child::node()[@codeListValue and not(@codeListValue=$codelist/codelist/entry/code)]">
            <option selected="selected" value="{@codeListValue}">
              ...
              <xsl:when test="contains($mode, 'radio')">
                <div class="helper helper-{$mode}">
                  <div>
                    ...
                    <label for="{$name}">
                      <xsl:value-of select="/root/gui/strings/nodata"/>
                    </label>
                  </div>
                  ...
                  <xsl:for-each select="$codelist/codelist/entry[not(@hideInEditMode)]">
                    <xsl:sort select="label"/>
                    <div>
                      <input class="md" type="radio" name="{$name}" id="{$name}{position()}"
                             value="{code}">
                        <xsl:if test="$isXLinked">
                          <xsl:attribute name="disabled">disabled</xsl:attribute>
                        </xsl:if>
                ...
                <select class="md" name="{$name}" id="{$name}">
                  <xsl:if test="$mode='select'">
                    <xsl:attribute name="size">
                      <xsl:value-of
                        select="if(count($codelist/codelist/entry[not(@hideInEditMode)]) &lt; $defaultSize)
            ...
            <xsl:if test="normalize-space($value)!=''">
              ...
              <xsl:choose>
                <xsl:when test="normalize-space($label)!=''">
                  ...
                  <xsl:value-of
                    select="concat(': ',$codelist/codelist/entry[code = $value]/description)"/>
                </xsl:when>
                <xsl:otherwise>
                  <b>
                    <xsl:value-of select="$value"/>
  ...
  <xsl:template match="gmd:MD_Keywords" mode="snippet-editor">
    ...
    * transformation: current transformation
    ...
    <xsl:variable name="listOfKeywords"
                  select="replace(replace(string-join(gmd:keyword/*[1], '#,#'), '''', '\\'''), '#', '''')"/>
    ...
    <xsl:variable name="listOfTransformations">'to-iso19139-keyword',
      'to-iso19139-keyword-with-anchor', 'to-iso19139-keyword-as-xlink'
    </xsl:variable>
    ...
    <xsl:variable name="thesaurusName"
                  select="gmd:thesaurusName/gmd:CI_Citation/gmd:title/gco:CharacterString"></xsl:variable>
  ...
  <xsl:template name="snippet-editor">
    ...
    -->
    <textarea id="thesaurusPicker_{$elementRef}_xml" name="_X{$elementRef}_replace" rows="" cols=""
              class="debug">
      ...
      <xsl:when test="$edit=true()">
        ...
        <xsl:apply-templates mode="complexElement" select=".">
          ...
          <xsl:with-param name="content">
            ...
            <xsl:variable name="thesaurusName"
                          select="gmd:MD_Keywords/gmd:thesaurusName/gmd:CI_Citation/gmd:title/gco:CharacterString"></xsl:variable>
      ...
      <xsl:otherwise>
        <xsl:apply-templates mode="simpleElement" select=".">
          ...
          <xsl:with-param name="title">
            ...
            <xsl:if
              test="gmd:MD_Keywords/gmd:thesaurusName/gmd:CI_Citation/gmd:title/gco:CharacterString">
              (<xsl:value-of
            ...
            <tr>
              <td align="center">
                <button class="content"
                        onclick="javascript:runFileDownloadSummary('{//geonet:info/uuid}','{/root/gui/strings/downloadSummary}')"
                        type="button">
            ...
            <xsl:variable name="fn">
              <xsl:choose>
               ...
               starts-with(gmd:protocol/gco:CharacterString,'OGC:OWS')">
                ...
                <xsl:otherwise>
                  <xsl:value-of
                    select="concat('app.getIMap().addWMSLayer([[''', $name, ''',''', $linkage, ''',''', $name, ''',''', $metadata_id, ''']])')"/>
                </xsl:otherwise>
              </xsl:choose>
            </xsl:variable>
        ...
        <xsl:apply-templates mode="simpleElement" select=".">
          ...
          <xsl:with-param name="text">
            ...
            <a href="{$linkage}" title="{$title}"
               onclick="runFileDownload(this.href, this.title); return false;">
        ...
        <xsl:call-template name="simpleElementGui">
          ...
          <xsl:with-param name="text">
            ...
            <xsl:variable name="relatedJsAction">
              <xsl:value-of
                select="concat('checkForFileUpload(&quot;',$fref,'&quot;, &quot;',$ref,'&quot;, ',
    ...
    <xsl:choose>
      <xsl:when test="$edit=true()">
        ...
        <xsl:call-template name="simpleElementGui">
          ...
          <xsl:with-param name="text">
            <button class="content"
                    onclick="Ext.getCmp('editorPanel').showFileUploadPanel({//geonet:info/id}, '{$ref}');"
                    type="button">
        ...
        <table width="100%">
          <tr>
            ...
            <td align="right">
              ...
              <button type="button"
                      onclick="javascript:doFileRemoveAction('{/root/gui/locService}/resources.del.new','{$ref}','{$access}','{$id}')">
  ...
  <xsl:template name="iso19139GeoPublisherButton">
    ...
    <xsl:if test="/root/gui/config/editor-geopublisher">
      ...
      <xsl:variable name="title">
        <xsl:apply-templates mode="escapeXMLEntities"
      ...
      <xsl:variable name="abstract">
        <xsl:apply-templates mode="escapeXMLEntities"
                             select="/root/gmd:MD_Metadata/gmd:identificationInfo/*/gmd:abstract/gco:CharacterString">
          <xsl:with-param name="includingCRLF" select="true()"/>
        </xsl:apply-templates>
      </xsl:variable>
  ...
  <xsl:template name="iso19139-brief">
    ...
    <geonet:info>
      ...
      <xsl:choose>
        <xsl:when
          test="/root/gui/env/harvester/enableEditing='false' and geonet:info/isHarvested='y' and geonet:info/edit='true'">
          <edit>false</edit>
        </xsl:when>
        <xsl:otherwise>
    ...
    <xsl:choose>
      <xsl:when test="$edit=true()">
        <xsl:variable name="text">
          ...
          <input
            onfocus="javascript:Ext.getCmp('editorPanel').showLinkedMetadataSelectionPanel('{$ref}', '');"
            class="md" type="text" name="_{$ref}" id="_{$ref}" value="{gco:CharacterString/text()}"
            size="20"/>
          <img src="../../images/find.png" alt="{/root/gui/strings/parentSearch}"
               title="{/root/gui/strings/parentSearch}"
        ...
        <xsl:apply-templates mode="simpleElement"
          ...
          <xsl:with-param name="text">&#160;
            ...
            <xsl:variable name="fileName"
                          select="gmd:MD_BrowseGraphic/gmd:fileName/gco:CharacterString"/>
            ...
            <div class="md-view">
              ...
              <span class="thumbnail">
                <a href="{$url}" target="thumbnail-view">
                  <xsl:value-of select="$imageTitle"/>
                </a>
              </span>
            </div>
  ...
  <xsl:template mode="get-thumbnail" match="gmd:MD_Metadata|*[@gco:isoType='gmd:MD_Metadata']">
    <xsl:apply-templates mode="get-thumbnail"
                         select="gmd:identificationInfo/*/gmd:graphicOverview"/>
  </xsl:template>
  ...
  <xsl:template mode="get-thumbnail" match="gmd:graphicOverview">
    ...
    <xsl:variable name="desc"
                  select="gmd:MD_BrowseGraphic/gmd:fileDescription/gco:CharacterString"/>
    <xsl:variable name="info"
                  select="ancestor::*[name(.) = 'gmd:MD_Metadata' or @gco:isoType='gmd:MD_Metadata']/geonet:info"></xsl:variable>

    <thumbnail>
      <href>
        <xsl:value-of select="geonet:get-thumbnail-url($fileName, $info, /root/gui/locService)"/>
      </href>
      <desc>
        <xsl:value-of select="$desc"/>
      </desc>
      <mimetype>
        <xsl:value-of select="gmd:MD_BrowseGraphic/gmd:fileType/gco:CharacterString"/>
      </mimetype>
      <type>
        <xsl:value-of
          select="if (geonet:contains-any-of($desc, ('thumbnail', 'large_thumbnail'))) then 'local' else ''"/>
      </type>
    </thumbnail>
  </xsl:template>
  ...
  >
    ...
    <xsl:variable name="class">
      <xsl:choose>
        <xsl:when test="name(.)='gmd:title' and name(../../../..)='gmd:identificationInfo'">title
        </xsl:when>
